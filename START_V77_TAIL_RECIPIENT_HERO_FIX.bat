...
echo - Preserves the stock head-client queue and game-state callback.
echo - Adds delivery only for the remaining linked clients.
...
set "LANIP=%~1"
if defined LANIP (
    echo Launching v77 with LAN IP %LANIP%
    "%PYEXE%" "%~dp0hon_v49_dashboard.py" "%LANIP%"
) else (
    echo Launching v77 with automatic LAN-IP detection.
