"""Build the ThorGor v65 linked-client state-block fix from verified k2.v57.

The v57 input is the verified ThorGor interoperability build. v65 keeps the
v63 all-client state-string delivery and routes state-block broadcasts directly
into the original queue body for every connection in the server's authoritative
linked-client list.
"""
...
OUTPUT_SHA256 = "82D0363C0BC853ECD60A3AD8A62E01E2BF0897EB1644364FBAC82C7E2B48ECAB"
...
def build(source: Path, target: Path) -> str:
    ...
    require(data, BLOCK_CAVE, b"\0" * 0xC0, "v65 code cave")
    ...
    if OUTPUT_SHA256 and result != OUTPUT_SHA256:
        raise ValueError(f"unexpected v65 output hash {result}")
...
def main(argv: list[str]) -> int:
    if len(argv) != 3:
        print("usage: build_k2_v65.py K2_V57_DLL OUTPUT_K2_DLL", file=sys.stderr)
        return 2
    try:
