...
echo ThorGor v76 world-ready joined-client hero-state fix
echo - Reconciles only state blocks 3 through 8.
echo - Waits until each client has fully loaded the world.
...
echo Installing and verifying K2 v76 plus cgame v61 in:
echo   %HON_HOME%
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0INSTALL_V76_PATCHES.ps1" -HonHome "%HON_HOME%"
if errorlevel 1 (
    echo ERROR: v76 DLL installation failed. Do not start a client.
    pause
    exit /b 3
)
...
set "LANIP=%~1"
if defined LANIP (
    echo Launching v76 with LAN IP %LANIP%
    "%PYEXE%" "%~dp0hon_v49_dashboard.py" "%LANIP%"
) else (
    echo Launching v76 with automatic LAN-IP detection.
