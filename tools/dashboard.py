"""ThorGor HoN 3.2.7 LAN sandbox dashboard and stack orchestrator."""
...
def _debug_output_dir() -> Path:
    candidates = (
        Path(os.environ.get("USERPROFILE", str(ROOT))) / "Desktop" / "ThorGor_Debug_Bundles",
        Path(os.environ.get("LOCALAPPDATA", os.environ.get("TEMP", str(ROOT)))) / "ThorGor" / "Debug_Bundles",
        ROOT / "debug_bundles",
    )
    for candidate in candidates:
        try:
            ...
            continue
    raise OSError("No writable ThorGor debug-bundle directory is available")
...
class Dashboard(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("ThorGor HoN 3.2.7 LAN Sandbox")
        ...
        ttk.Label(self, text="ThorGor HoN 3.2.7 LAN Sandbox", style="Title.TLabel").pack(anchor="w", padx=22, pady=(17, 0))
        ttk.Label(self, text=f"Isolated master, chat, game and dedicated-server stack  •  LAN {LAN_IP} ({LAN_IP_SOURCE})", style="Sub.TLabel").pack(anchor="w", padx=23, pady=(1, 0))
