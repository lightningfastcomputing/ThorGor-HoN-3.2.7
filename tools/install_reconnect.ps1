...
# Resolve this checkout, including paths containing spaces. Older numbered
# installers intentionally remain historical; they reference the old workspace.
...
$log = Join-Path $logDirectory 'install_reconnect_v20.log'
...
try {
    ...
    $dashboard = Start-Process -FilePath $python `
        ...
        -RedirectStandardOutput (Join-Path $logDirectory 'stack_v20.out.log') `
        -RedirectStandardError (Join-Path $logDirectory 'stack_v20.err.log')
    ...
    Write-Error "Reconnect installation failed. See $log"
    exit 1
}
