...
$log = Join-Path $logDirectory 'install_reconnect_v23.log'
...
try {
    ...
    $dashboard = Start-Process -FilePath $python `
        ...
        -RedirectStandardOutput (Join-Path $logDirectory 'stack_v23.out.log') `
        -RedirectStandardError (Join-Path $logDirectory 'stack_v23.err.log')
    ...
    Write-Error "Reconnect v23 installation failed. See $log"
    exit 1
}
