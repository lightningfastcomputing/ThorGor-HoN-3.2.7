...
$log = Join-Path $logDirectory 'install_reconnect_v25.log'
...
try {
    ...
    $dashboard = Start-Process -FilePath $python `
        ...
        -RedirectStandardOutput (Join-Path $logDirectory 'stack_v25.out.log') `
        -RedirectStandardError (Join-Path $logDirectory 'stack_v25.err.log')
    ...
    Write-Error "Reconnect v25 installation failed. See $log"
    exit 1
}
