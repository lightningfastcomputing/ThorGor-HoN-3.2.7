"""Build ThorGor K2 v75 from the verified stable v65 DLL.
...
v75 replaces only that periodic check.  It compares blocks 3..8 independently
for every linked client and calls K2's original guarded QueueStateBlock entry
point.  The original call both queues the block and advances that client's
...
OUTPUT_SHA256 = "9D731944738C6CA014CB71F25F82DCE8634522247AB935513E2F5A0889C0BFF3"
...
def build(source: Path, target: Path) -> str:
    ...
    require(data, RECONCILE_WRAPPER, b"\0" * 0x80, "v75 reconciliation cave")

    reconcile = bytearray.fromhex(
        "60"                # pushad
        "83fb03"            # cmp ebx,3
    )
    below_guard = RECONCILE_WRAPPER + len(reconcile)
    reconcile += b"\x0f\x82\0\0\0\0"  # jb done
    reconcile += bytes.fromhex("83fb08")  # cmp ebx,8
    ...
    if len(reconcile) > 0x80:
        raise ValueError(f"v75 reconciliation stub is too large: {len(reconcile)}")
...
def main(argv: list[str]) -> int:
    if len(argv) != 3:
        print("usage: build_k2_v75.py K2_V65_DLL OUTPUT_K2_DLL", file=sys.stderr)
        return 2
    try:
