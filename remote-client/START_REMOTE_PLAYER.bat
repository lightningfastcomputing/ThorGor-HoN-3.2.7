...
echo.
echo ThorGor v77 remote LAN player
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  "$p=Start-Process powershell.exe -Verb RunAs -Wait -PassThru -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-File','""%~dp0..\INSTALL_V77_PATCHES.ps1""','-HonHome','""%HON_HOME%""'; exit $p.ExitCode"
if errorlevel 1 (
