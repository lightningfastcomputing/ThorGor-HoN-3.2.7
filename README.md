...
- Windows
- Git on `PATH`
- Python 3.10 or newer (Python 3.14 is tested; install with `winget install --exact --id Python.Python.3.14`)
...
The repository supplies no HoN binaries. During startup, the installers verify the user-supplied 3.2.7.1 DLL hashes, generate the v57/v61 interoperability patches locally, preserve verified backups, configure the LAN firewall rules, provision the disposable test accounts, and start the dashboard. The one-liner elevates the batch file directly, avoiding an extra `cmd.exe /c` quoting layer. The launcher verifies that the dashboard stays alive through initialization; if it cannot open, the elevated window remains visible with the error and writes `dashboard_logs/dashboard-startup.stderr.log`.
