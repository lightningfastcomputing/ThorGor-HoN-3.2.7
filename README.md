...
```powershell
$dir=Join-Path $env:USERPROFILE 'ThorGor-HoN-3.2.7'; if(Test-Path (Join-Path $dir '.git')){git -C $dir pull --ff-only}else{git clone https://github.com/lightningfastcomputing/ThorGor-HoN-3.2.7.git $dir}; if($LASTEXITCODE -ne 0){throw 'Git acquire/update failed'}; Start-Process cmd.exe -Verb RunAs -WorkingDirectory $dir -ArgumentList '/c',('"' + (Join-Path $dir '1_START_V61_COMPLETE_REGISTRY_GUARD.bat') + '"')
```
