...
$candidate = Join-Path $HonHome 'k2.dll.thorgor_v66.new'
$builder = Join-Path $PSScriptRoot 'patches\build_k2_v66_state_delivery.py'
$v57Hash = '6F5FC1F7BF4E01CDEB0360A6F703299F5422F2C06A104FADCB24FD96A546B8DF'
$v66Hash = '2BC131F1C40D9F84CAD288426B14B0DB1EE58E43FC64DB86FF5AEFFC82D58657'
...
if ((Hash $target) -eq $v66Hash) {
    Write-Host 'K2 v66 per-client state reconciliation is already installed.' -ForegroundColor DarkGray
    exit 0
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'K2 v66 patch builder failed.' }
    if ((Hash $candidate) -ne $v66Hash) { throw 'Generated K2 v66 hash verification failed.' }
    Move-Item -LiteralPath $candidate -Destination $target -Force
} finally {
    Remove-Item -LiteralPath $candidate -Force -ErrorAction SilentlyContinue
}
Write-Host 'Generated and installed K2 v66 per-client state reconciliation from the verified v57 baseline.' -ForegroundColor Green
